...
checkpoint_path = os.path.join(checkpoint_dir, "letac_mpc_checkpoint.pt")
resume_training = True
save_every = 1

def save_checkpoint(path, epoch, cnn_encoder, mpc_layer, optimizer):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    torch.save(
        {
            "epoch": epoch,
            "cnn_encoder_state": cnn_encoder.state_dict(),
            "mpc_layer_state": mpc_layer.state_dict(),
            "optimizer_state": optimizer.state_dict(),
        },
        path,
    )

def load_checkpoint(path, device, cnn_encoder, mpc_layer, optimizer):
...
start_epoch = 0
if resume_training:
    start_epoch = load_checkpoint(checkpoint_path, device, cnn_encoder, MPC_layer, optimizer)
...
for epoch in range(start_epoch, epochs):
    ...
    if (epoch + 1) % save_every == 0:
        save_checkpoint(checkpoint_path, epoch, cnn_encoder, MPC_layer, optimizer)
